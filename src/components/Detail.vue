<template>
  <div class="detail">
    ...
    <div class="content">
      <div class="header clear"><h2><img style="max-width: 100px" :src="detail.detail && baseUrl + detail.detail.auth_icon" alt=""/></h2><p>{{detail.detail && detail.detail.auth}}</p></div>
      <div class="cont">
        <h3>{{detail.title}}</h3>
        <div class="time"><p>{{detail.time | date}}<span><img src="../assets/img/zan.png" alt=""/></span></p></div>
        <div class="text-box" v-html="detail.detail && detail.detail.content"></div>
      </div>
    </div>
    <div class="foot-btn">
      <ul>
...
export default {
  ...
  mounted(){
    ...
    axios({
      ...
      res=>this.detail=res.data.page_data
    )
  }
}
</script>
